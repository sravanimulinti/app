...
class AppStore extends Component {
  state = {searchInput: '', activeElement: tabsList[0].tabId}
  ...
  getResult = tabId => {
    this.setState({activeElement: tabId})
  }
  ...
  listItem = () => {
    ...
    const searchList = appsList.filter(eachEle =>
      eachEle.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    return searchList
  }
  ...
  render() {
    ...
    const {searchInput, activeElement} = this.state

    const searchList = this.listItem
    const filterElement = this.appListItem(searchList)
    return (
      <div className="bg">
        ...
        <ul className="order">
          {tabsList.map(eachObj => (
            <TabItem
              key={eachObj}
              appDetails={eachObj.tabId}
              appListItem={this.appListItem}
              isActive={activeElement === eachObj.tabId}
            />
          ))}
        </ul>
        <ul className="order">
          {filterElement.map(each => (
            <AppItem key={each.appId} detailApp={each} />
          ))}
        </ul>
      </div>
    )
